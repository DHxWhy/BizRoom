---
version: "3.0.0"
created: "2026-03-14 07:00"
updated: "2026-03-14 08:00"
---

# AI Model Benchmark Report — BizRoom.ai

> 7개 모델의 응답 속도·품질·비용을 동일 프롬프트로 비교 측정한 결과입니다.
> gpt-realtime-1.5 정식 모델 + gpt-4o-realtime-preview 모두 실측 완료.

---

## 테스트 조건

| 항목         | 값                                                          |
| ------------ | ----------------------------------------------------------- |
| 프롬프트     | "Q2 마케팅 예산 3000만원 채널별 배분 전략 제안"              |
| System       | "You are Hudson, COO. Respond in Korean. 3 sentences max."  |
| max_tokens   | 300                                                         |
| temperature  | 0.5 (Realtime: 0.6 — 최소값 제한)                           |
| 측정 방식    | Direct API 호출 (네트워크 왕복 포함)                        |
| 측정 일시    | 2026-03-14                                                  |

---

## 단일 에이전트 응답 — 전체 비교

| 순위 | 모델                              | Provider  | 응답 시간   | TTFB       | 토큰 (in/out) | 품질   |
| ---- | --------------------------------- | --------- | ----------- | ---------- | ------------- | ------ |
| 1    | **gpt-4o-realtime-preview**       | OpenAI    | **1.95초**  | **0.93초** | -             | ★★★★  |
| 2    | **gpt-4o**                        | OpenAI    | **2.1초**   | -          | 46 / 84       | ★★★★  |
| 3    | **claude-haiku-4-5-20251001**     | Anthropic | **2.3초**   | -          | 47 / 208      | ★★★☆  |
| 4    | **gpt-realtime-1.5**              | OpenAI    | **2.52초**  | **1.46초** | -             | ★★★★☆ |
| 5    | **gpt-4o-mini**                   | OpenAI    | **2.8초**   | -          | 46 / 80       | ★★★☆  |
| 6    | **claude-sonnet-4-6**             | Anthropic | **4.8초**   | -          | 48 / 214      | ★★★★  |
| 7    | **claude-opus-4-6**               | Anthropic | ~7.0초      | ~4.5초     | 60 / 281      | ★★★★★ |

> TTFB = Time To First Byte (첫 토큰 도착까지 시간). Realtime 모델은 WebSocket 프로토콜.

---

## Realtime 모델 비교 — 실측 결과

| 항목             | gpt-4o-realtime-preview            | gpt-realtime-1.5 (정식)            |
| ---------------- | ---------------------------------- | ---------------------------------- |
| **TTFB**         | **926ms**                          | **1,460ms**                        |
| **전체 완료**    | **1,950ms**                        | **2,520ms**                        |
| **Delta 이벤트** | 117개                              | 101개                              |
| **응답 품질**    | 60/30/10 배분, 간결                | 40/30/20/10 4단계 배분, 더 상세    |
| **프로토콜**     | WebSocket                          | WebSocket                          |
| **특이사항**     | temperature 최소 0.6               | temperature 최소 0.6               |

### 응답 샘플

**gpt-4o-realtime-preview (TTFB 926ms)**:
> 30백만 원의 예산이라면 디지털 마케팅에 60%를 배정해 온라인 광고와 소셜 미디어 캠페인을 강화하는 것이 좋습니다. 나머지 30%는 오프라인 프로모션이나 이벤트를 통해 브랜드 인지도를 높이는 데 사용하고, 10%는 데이터 분석과 마케팅 성과 측정에 투자하면 효율적입니다.

**gpt-realtime-1.5 (TTFB 1,460ms)**:
> 30백만 원 중 40%는 검색광고와 SNS 광고에 집중해 브랜드 인지도와 전환을 높이겠습니다. 30%는 인플루언서 협업과 콘텐츠 제작에 투입해 신뢰도를 확보하고, 20%는 이메일·리타게팅 캠페인으로 재구매를 유도하겠습니다. 나머지 10%는 오프라인 이벤트나 PR에 사용해 고객 접점을 다양화할 계획입니다.

### 판정
- **속도 우선**: `gpt-4o-realtime-preview` (TTFB 0.93초 — 1초 미만)
- **품질 우선**: `gpt-realtime-1.5` (4단계 상세 배분, 더 구체적인 전략)
- **음성 회의 권장**: `gpt-realtime-1.5` — 0.5초 차이로 품질이 확연히 높음

### Realtime이 빠른 이유
- **WebSocket 연결 유지**: HTTP 요청/응답 오버헤드 없음
- **양방향 스트리밍**: 서버가 준비되는 즉시 delta 전송 시작
- **음성 최적화 아키텍처**: 저지연 추론 엔진

---

## Backend 경유 시뮬레이션 (3 에이전트 순차 vs 병렬)

### 현재: 순차 호출

```
Hudson (COO) ────[4.8초]────▶
                              Amelia (CFO) ────[4.8초]────▶
                                                            Yusef (CMO) ────[4.8초]────▶
|──────────────────── 총 14.4초 ─────────────────────────────────────────────────────────|
```

### 개선안: 병렬 호출

```
Hudson (COO)  ────[4.8초]────▶
Amelia (CFO)  ────[4.8초]────▶
Yusef  (CMO)  ────[4.8초]────▶
|─────── 총 4.8초 ──────────|
```

| 방식   | Sonnet 4.6 (3명) | Haiku 4.5 (3명) | GPT-4o (3명)   | Realtime (1명) |
| ------ | ---------------- | --------------- | -------------- | -------------- |
| 순차   | ~14초            | ~7초            | ~6초           | -              |
| 병렬   | **~5초**         | **~2.5초**      | **~2.5초**     | **~1.8초**     |

> 병렬 호출 시 가장 느린 에이전트 기준으로 전체 시간 결정. SSE 스트리밍은 에이전트별 인터리빙 가능.

---

## 모델별 특성 매트릭스

| 기준           | Opus 4.6  | Sonnet 4.6 | Haiku 4.5 | GPT-4o    | GPT-4o-mini | RT-preview | RT-1.5    |
| -------------- | --------- | ---------- | --------- | --------- | ----------- | ---------- | --------- |
| **속도**       | ★★☆☆☆    | ★★★☆☆     | ★★★★☆    | ★★★★☆    | ★★★★☆      | ★★★★★     | ★★★★☆    |
| **품질**       | ★★★★★    | ★★★★☆     | ★★★☆☆    | ★★★★☆    | ★★★☆☆      | ★★★★☆     | ★★★★☆    |
| **비용**       | $$$$$     | $$$        | $          | $$$       | $           | $$         | $$        |
| **한국어**     | 매우 자연 | 자연       | 자연      | 자연      | 자연        | 자연       | 자연      |
| **프로토콜**   | REST      | REST       | REST      | REST      | REST        | WebSocket  | WebSocket |
| **적합 용도**  | 파일 생성 | BigScreen  | 모니터    | 일반 회의 | 빠른 응답   | 음성 대화  | 음성 대화 |

---

## 응답 샘플 (동일 프롬프트)

### Realtime (gpt-4o-realtime-preview) — 1.8초
> Q2 마케팅 예산 3천만 원이라면, 디지털 채널에 60%를 집중 투자해 온라인 광고와 SNS 캠페인을 강화하는 게 좋습니다. 나머지 30%는 오프라인 이벤트나 프로모션에 할당해 고객 체험을 유도하고 브랜드 인지도를 높일 수 있습니다. 마지막 10%는 데이터 분석과 A/B 테스트에 사용해 전략의 효율성을 지속적으로 개선하는 데 활용하는 걸 추천합니다.

### Sonnet 4.6 — 4.8초
> Q2 마케팅 예산 3,000만 원 채널 배분 전략: 퍼포먼스 마케팅(Meta/Google) 40% (1,200만 원), 콘텐츠/인플루언서 35% (1,050만 원), SEO/이메일/리타겟팅 25% (750만 원)

### Haiku 4.5 — 2.3초
> 디지털 채널(소셜, 검색, 디스플레이) 50%(1,500만 원), 전통 매체(TV, 라디오) 25%(750만 원), 브랜드 활동 및 기타 채널에 25%(750만 원) 할당하여 효과적으로 예산을 확보하겠습니다.

### GPT-4o — 2.1초
> 온라인 채널에 60%, 오프라인 채널에 40%를 배분합니다. 온라인에서는 소셜 미디어와 검색 광고를 병행하고, 오프라인에서는 이벤트와 협업 사례를 활용합니다.

---

## 권장 설정

### 해커톤 데모용 (속도+인상 극대화)

```bash
# 음성: Realtime (TTFB <1초)
# 텍스트: GPT-4o (2초) 또는 Haiku (2.3초)
# 시각화: Haiku (빠른 파싱)
ANTHROPIC_MODEL_PREMIUM  = claude-haiku-4-5-20251001
ANTHROPIC_MODEL_BALANCED = claude-haiku-4-5-20251001
ANTHROPIC_MODEL_FAST     = claude-haiku-4-5-20251001
```

예상: 에이전트당 ~2초, 3명 순차 ~7초, 병렬 시 ~2.5초

### 프로덕션 (품질·속도 균형)

```bash
ANTHROPIC_MODEL_PREMIUM  = claude-sonnet-4-6
ANTHROPIC_MODEL_BALANCED = claude-haiku-4-5-20251001
ANTHROPIC_MODEL_FAST     = claude-haiku-4-5-20251001
```

예상: 에이전트당 ~3-5초, 3명 순차 ~12초, 병렬 시 ~5초

### 음성 회의 최적 + 소피아 동적 모델

```bash
# Live 모드 PTT → gpt-realtime-1.5 (WebSocket, TTFB 1.5초, 품질↑)
# 텍스트 채팅 → claude-sonnet-4-6 (~5초)
# Sophia 의장모니터/key_points → claude-haiku-4-5-20251001 (빠른 응답 ~2초)
# Sophia BigScreen 시각화 → claude-sonnet-4-6 (Show me 코드 생성 품질)
# Sophia 파일 생성 (PPT/Excel) → claude-opus-4-6 (구조화 품질 최대)
```

---

## 발견된 이슈 및 해결

| #  | 이슈                                       | 상태     | 해결 방법                                          |
| -- | ------------------------------------------ | -------- | -------------------------------------------------- |
| 1  | Opus 모델명 `claude-opus-4-6-20250929` 404 | **해결** | 환경변수로 정확한 모델 ID 오버라이드               |
| 2  | OpenAI `insufficient_quota`                | **해결** | 사용자 크레딧 충전                                 |
| 3  | Haiku 모델명 `claude-haiku-4-5` 무응답     | **해결** | 정확한 ID: `claude-haiku-4-5-20251001`             |
| 4  | Realtime temperature < 0.6 에러            | **해결** | 최소 0.6 적용                                      |
| 5  | 순차 호출 병목                             | **해결** | Promise.all 병렬 호출로 6x 단축 (배포됨)          |
| 6  | gpt-realtime-1.5 미테스트                  | **해결** | WebSocket 실측 완료 (TTFB 1.46초, Total 2.52초)   |

---

## 소피아 동적 모델 라우팅 권장 설정

| 소피아 태스크         | 모델                          | 이유                           | 예상 시간 |
| --------------------- | ----------------------------- | ------------------------------ | --------- |
| 의장 모니터 (key_points) | claude-haiku-4-5-20251001  | 속도 최우선, 간단한 요약       | ~2초      |
| BigScreen 시각화      | claude-sonnet-4-6             | Show me 코드 생성 품질 필요    | ~5초      |
| 파일 생성 (PPT/Excel) | claude-opus-4-6               | 구조화·정밀도 최대             | ~7초      |
| 회의록 생성           | claude-sonnet-4-6             | 품질+속도 균형                 | ~5초      |

---

## 소피아 기능별 벤치마크

### 시각화 생성 (BigScreenRenderData JSON)

| 모델              | 시간       | 토큰 (in/out) | JSON 유효 | 품질                                 |
| ----------------- | ---------- | ------------- | --------- | ------------------------------------ |
| **Haiku 4.5**     | **1.6초**  | 193 / 198     | ✅         | 기본 구조, 충분한 데이터             |
| **Sonnet 4.6**    | **10.5초** | 194 / 795     | ✅         | 상세 메타데이터, subtitle, source 포함 |

> **권장: Haiku 4.5** — 의장 모니터 + 빠른 시각화에 6.5x 빠름, 품질 충분

### Show me HTML 시각화 (Claude 2026-03-12 신기능)

| 모델              | 시간       | 토큰 (in/out) | HTML 생성 | 품질                                       |
| ----------------- | ---------- | ------------- | --------- | ------------------------------------------ |
| **Sonnet 4.6**    | **11.8초** | 192 / 1000    | ✅         | 완전한 HTML+CSS+SVG 파이차트, 인터랙티브   |

> BigScreen에 HTML 직접 렌더링 가능. JSON→차트 변환 불필요. `max_tokens` 증가 시 더 풍부한 시각화.

### 회의록 생성

| 모델              | 시간       | 토큰 (in/out) | JSON 유효 | 품질                                       |
| ----------------- | ---------- | ------------- | --------- | ------------------------------------------ |
| **Sonnet 4.6**    | **12.6초** | 227 / 1000    | ✅         | 상세 구조, 참석자/안건/결정/액션아이템     |
| **Opus 4.6**      | **9.7초**  | 227 / 667     | ✅         | 간결하지만 정확한 구조                     |

> **권장: Opus 4.6** — 회의록은 품질 최우선, Opus가 더 빠르고 정확

### 소피아 태스크별 최종 권장 모델

| 소피아 태스크                | 최적 모델                   | 응답 시간 | 근거                               |
| ---------------------------- | --------------------------- | --------- | ---------------------------------- |
| 의장 모니터 (key_points)     | **claude-haiku-4-5-20251001** | ~1.6초  | 속도 최우선, 간단한 요약           |
| BigScreen 시각화 (JSON)      | **claude-haiku-4-5-20251001** | ~1.6초  | 빠른 차트 생성, 품질 충분          |
| BigScreen Show me (HTML)     | **claude-sonnet-4-6**       | ~12초     | HTML/SVG 코드 생성 품질            |
| 파일 생성 (PPT/Excel/회의록) | **claude-opus-4-6**         | ~10초     | 구조화 정밀도 최대                 |

---

## 턴매니저 타이밍 설정

| 파라미터                  | 현재 값    | 설명                           |
| ------------------------- | ---------- | ------------------------------ |
| `CHAIRMAN_FLUSH_MS`       | 300ms      | 의장 발화 후 AI 트리거 지연    |
| `MEMBER_FLUSH_MS`         | 2,000ms    | 멤버 발화 후 추가 입력 대기    |
| `INTER_AGENT_GAP_MS`      | 1,500ms    | 에이전트 간 응답 간격          |
| `MAX_AGENTS_PER_TURN`     | **2**      | 턴당 최대 응답 에이전트 수     |
| `MAX_FOLLOW_UP_ROUNDS`    | 2          | A2A 후속 라운드 최대           |
| `HUMAN_CALLOUT_TIMEOUT_MS`| 30,000ms   | 의장 응답 타임아웃             |

### 턴매니저 예상 타이밍 (Sonnet 4.6 기준, 병렬 호출 적용 후)

```
사용자 발화 → [300ms flush] → 토픽 분류 → 에이전트 2명 병렬 호출
                                            ├─ Agent A: ~5초
                                            └─ Agent B: ~5초
                                                          → [1.5초 gap] → 완료
총: 300ms + 5초 + 1.5초 = ~6.8초 (2 agents)
```

### Realtime 음성 경로 예상 타이밍

```
PTT 누르기 → 음성 입력 → [300ms flush] → gpt-realtime-1.5 WebSocket
                                          → [1.46초 TTFB] → 스트리밍 응답
총: 300ms + 1.46초 = ~1.76초 TTFB
```

---

## 현재 적용 설정

```
ANTHROPIC_MODEL_PREMIUM  = claude-sonnet-4-6       (에이전트 응답)
ANTHROPIC_MODEL_BALANCED = claude-sonnet-4-6       (시각화)
ANTHROPIC_MODEL_FAST     = claude-haiku-4-5        (파싱/분류)
에이전트 병렬 호출       = Promise.all (배포됨)
Realtime 음성            = gpt-realtime-1.5        (ModelRouter 기본값)
```
